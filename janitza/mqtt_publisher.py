...
class MQTTPublisher:
    ...
    def __init__(self, config: MQTTConfig, registers: List[SelectedRegister],
        ...
        Args:
            ...
            registers: List of selected registers with topic configuration
            publish_mode: 'changed' (only publish changes) or 'all'
        """
        self.config = config
    ...
    def _build_topic(self, register: SelectedRegister) -> str:
        ...
        if register.mqtt_topic:
            # Use custom topic from configuration
            return f"{self.config.topic_prefix}/{register.mqtt_topic}"
        else:
            # Generate from register name
    ...
    def publish_ha_discovery(self) -> int:
        ...
        Returns number of configs published.
        """
    ...
    def reconnect(self) -> bool:
        ...
        # Setup new client with updated config
        self._setup_client()

        # Connect
        if self._try_connect():
            logger.info("MQTT reconnected successfully")
            # Republish HA discovery if enabled
